(() => {
  ...
  function showRoleState() {
    const admin = isAdmin();
    el('contractReadOnlyNotice')?.classList.toggle('hidden', admin);
    el('contractAdminWorkspace')?.classList.toggle('hidden', !admin);
    if (admin) renderMemberList();
  }
  ...
  async function downloadContract() {
    if (!window.LSOPermissions?.requireAdmin?.('Only an Administrator can generate a membership contract.')) return;
    setMessage();
