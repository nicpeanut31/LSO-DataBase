(() => {
  ...
  async function registerServiceWorker(){
    ...
    try{
      ...
      await Promise.all(keys.filter(key=>key.startsWith('lso-website-')&&!key.includes('20260724-member-information-14')).map(key=>caches.delete(key)));
    }catch{}
    const secure=window.isSecureContext||['localhost','127.0.0.1'].includes(location.hostname);
    if(!secure)return;
    try{
      const registration=await navigator.serviceWorker.register('./service-worker-member-info-v14.js',{scope:'./',updateViaCache:'none'});
