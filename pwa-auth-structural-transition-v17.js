  ...
  async function registerServiceWorker(){
    ...
    try{
      ...
      await Promise.all(registrations.filter(registration=>{
        ...
        return script && !script.includes('service-worker-auth-structural-transition-v17.js');
      }).map(registration=>registration.unregister()));
      const keys=await caches.keys();
      await Promise.all(keys.filter(key=>key.startsWith('lso-website-')&&!key.includes('20260724-auth-structural-transition-17')).map(key=>caches.delete(key)));
    }catch{}
    const secure=window.isSecureContext||['localhost','127.0.0.1'].includes(location.hostname);
    if(!secure)return;
    try{
      const registration=await navigator.serviceWorker.register('./service-worker-auth-structural-transition-v17.js',{scope:'./',updateViaCache:'none'});
